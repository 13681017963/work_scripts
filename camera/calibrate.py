...
# 根据棋盘格规格，生成世界坐标
def get_obj_points(checker_board: tuple, square_size: tuple):
    """
    生成世界坐标
    根据标定版规格，自动生成世界坐标
    ...
    :param square_size: (row, col) 棋盘格大小，单位mm
    ...
    obj_points[0, :, 0] *= square_size[0]
    obj_points[0, :, 1] *= square_size[1]
    return obj_points
...
# 报错！！！find_image_points方法导致,原因没找到
# 用上面的方法替换，区别在于新方法使用了所有亚像素角点，而不是只用四个。采用EPNP算法有效利用多点信息，理论上精度更高
# 使用棋盘格标定相机外参
def old_cal_outside_image_monocular(obj_p, img_, checker_board, mtx, dist):
    """
    根据内参，图像，世界坐标，求出对于当前世界坐标系下的外参（旋转、平移）
    :param obj_p: 角点的世界坐标，shape:[]
    :param img_: 用于标定的图片
    ...
    try:
        ...
        ret, rvec, tvec = cv2.solvePnP(obj_p, corners.squeeze(), mtx, dist)
        # 将旋转向量转换为旋转矩阵
        rmat = cv2.Rodrigues(rvec)[0]
        # 返回相关信息
        return True, rvec, rmat, tvec
...
def cal_outside_punctuation_fisheye(obj_point, img_point, mtx, dist):
    ...
    try:
        ...
        ret, rvec, tvec = cv2.solvePnP(obj_point, img_point.squeeze(), np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), None)
        # 将旋转向量转换为旋转矩阵
        rmat = cv2.Rodrigues(rvec)[0]
        # 返回相关信息
        return True, rvec, rmat, tvec
