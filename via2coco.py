    ...
    for idx, v in enumerate(mmcv.track_iter_progress(data_infos.values())):
        ...
        for region in v["regions"]:
            ...
            if ratype in ignore_keys:
                continue
            group_id = region['region_attributes']['groupId']
...
given_id_dict = {'pig':0}
ignore_keys = ['pigsty_corral', 'countPassage', 'square_area']

id_dict = parse_category_id(LOKI_ann_file_train, LOKI_ann_file_val, given_id_dict, ignore_keys)
convert_balloon_to_coco(LOKI_ann_file=LOKI_ann_file_train, COCO_out_json=COCO_out_json_train, image_prefix=image_prefix_train, category_id_dict = id_dict)
convert_balloon_to_coco(LOKI_ann_file=LOKI_ann_file_val, COCO_out_json=COCO_out_json_val, image_prefix=image_prefix_val, category_id_dict = id_dict)
